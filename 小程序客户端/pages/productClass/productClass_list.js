...
Page({
  ...
  // 加载套餐类别列表
  listProductClass: function () {
    var self = this
    var url = config.basePath + "api/productClass/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          productClasss: self.data.productClasss.concat(res.data.list),
  ...
  removeProductClass: function (e) {
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/productClass/delete/" + classId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除套餐类别后客户端同步删除数据
            var productClasss = self.data.productClasss;
            for (var i = 0; i < productClasss.length; i++) {
              if (productClasss[i].classId == classId) {
                productClasss.splice(i, 1)
                break
              }
            }
            self.setData({ productClasss: productClasss })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
