...
public class OrderInfoController extends BaseController {
	...
	public void list(@ModelAttribute("productObj") Product productObj,String orderStateObj,String receiveName,String telephone,@ModelAttribute("userObj") UserInfo userObj,String orderTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
	...
	public String frontlist(@ModelAttribute("productObj") Product productObj,String orderStateObj,String receiveName,String telephone,@ModelAttribute("userObj") UserInfo userObj,String orderTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (orderStateObj == null) orderStateObj = "";
		if (receiveName == null) receiveName = "";
		if (telephone == null) telephone = "";
		if (orderTime == null) orderTime = "";
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(productObj, orderStateObj, receiveName, telephone, userObj, orderTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(productObj, orderStateObj, receiveName, telephone, userObj, orderTime);
	    /*获取到总的页码数目*/
	    int totalPage = orderInfoService.getTotalPage();
	...
	public String shopFrontlist(@ModelAttribute("productObj") Product productObj,String orderStateObj,String receiveName,String telephone,@ModelAttribute("userObj") UserInfo userObj,String orderTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (orderStateObj == null) orderStateObj = "";
		if (receiveName == null) receiveName = "";
		if (telephone == null) telephone = "";
		...
		String shopUserName = (String)session.getAttribute("user_name");
		
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(shopUserName, orderStateObj, receiveName, telephone, userObj, orderTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(shopUserName, orderStateObj, receiveName, telephone, userObj, orderTime);
	    /*获取到总的页码数目*/
	    int totalPage = orderInfoService.getTotalPage();
	    /*当前查询条件下总记录数*/
	    int recordNumber = orderInfoService.getRecordNumber();
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "OrderInfo/orderInfo_shopFrontquery_result"; 
	}
	...
	public void OutToExcel(@ModelAttribute("productObj") Product productObj,String orderStateObj,String receiveName,String telephone,@ModelAttribute("userObj") UserInfo userObj,String orderTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(orderStateObj == null) orderStateObj = "";
        if(receiveName == null) receiveName = "";
        if(telephone == null) telephone = "";
        if(orderTime == null) orderTime = "";
        List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(productObj,orderStateObj,receiveName,telephone,userObj,orderTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "OrderInfo信息记录"; 
        String[] headers = { "订单编号","打印套餐","订单数量","订单总金额","支付方式","订单状态","收货人","收货人电话","下单用户","下单时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<orderInfoList.size();i++) {
        	OrderInfo orderInfo = orderInfoList.get(i); 
        	dataset.add(new String[]{orderInfo.getOrderId() + "",orderInfo.getProductObj().getProductName(),orderInfo.getOrderNum() + "",orderInfo.getTotalMoney() + "",orderInfo.getPayWay(),orderInfo.getOrderStateObj(),orderInfo.getReceiveName(),orderInfo.getTelephone(),orderInfo.getUserObj().getName(),orderInfo.getOrderTime()});
        }
        /*
