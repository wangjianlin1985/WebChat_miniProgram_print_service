	...
	product_manage_tool = {
		init: function() {
			...
			$.ajax({
				...
				type : "post",
				success : function (data, response, status) {
					$("#printShopObj_shopUserName_query").combobox({ 
					    valueField:"shopUserName",
					    textField:"shopName",
					    panelHeight: "200px",
				        editable: false, //不允许手动输入 
					});
					data.splice(0,0,{shopUserName:"",shopName:"不限制"});
					$("#printShopObj_shopUserName_query").combobox("loadData",data); 
				}
			});
		},
		reload : function () {
		...
		search: function() {
			...
			queryParams["printShopObj.shopUserName"] = $("#printShopObj_shopUserName_query").combobox("getValue");
